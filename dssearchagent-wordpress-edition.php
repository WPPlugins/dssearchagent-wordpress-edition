...
if (!$activePlugins)
	$activePlugins = array();
$oldPluginName = "dssearchagent-wordpress-edition/dssearchagent-wordpress-edition.php";

if (in_array($oldPluginName, $activePlugins)) {
	$file = __FILE__;
	$dir = dirname($file);
	
	unset($active_plugins[$oldPluginName]);
	$activePlugins[] = "dsidxpress/dsidxpress.php";
	wp_cache_flush();
	update_option('active_plugins', $activePlugins);
	delete_transient('update_plugins');
	
	eval(
		"rename('$file', '" . $dir . "/" . str_replace('dssearchagent-wordpress-edition', 'dsidxpress', basename(__FILE__)) . "'); " .
		"rename('$dir', '" . str_replace('dssearchagent-wordpress-edition', 'dsidxpress', $dir) . "'); "
	);
}
?>